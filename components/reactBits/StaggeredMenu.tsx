...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? 'fixed top-0 left-0 h-screen overflow-hidden' : 'w-full h-full'}`}
    >
      <div
        className={`sm-scope z-40 fixed top-0 left-0 w-screen h-screen ${
      ...
      >
        ...
        >
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo"></div>

          <button
            ref={toggleBtnRef}
            className={`sm-toggle relative inline-flex items-center gap-[0.3rem] mt-10 -mr-6 sm:-mr-3 bg-black px-3 py-4 rounded-[10px] border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ${
        ...
        <aside
          ...
          ref={panelRef}
          className={`staggered-menu-panel ${ open ? 'pointer-events-auto' : 'pointer-events-none'} absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px] pointer-events-auto`}
                ...
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <Link
                      ...
                      data-index={idx + 1}
                    >
                      <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform font-bold text-3xl">
                        {it.label}
                      </span>
                    </Link>
                  </li>
